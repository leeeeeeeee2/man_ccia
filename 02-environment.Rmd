...
```{block, type='rmdtip'}
We recommend you create a new Anaconda environment for this manual. You can accomplish this by writing the following commands from the base Anaconda environment (i.e from the Anaconda Prompt on Windows or after running `conda activate` on MacOS or Linux). 

`conda create -n "man_ccia" python>=3.7.1 matplotlib netCDF4 pandas seaborn>=0.9.0`

If you plan to use Jupyter Notebook, your new environment won't be immediately available, however, you can install a "kernel" for your new environment by issuing the following commands: 

    conda activate man_ccia
    conda install ipykernel
    python -m ipykernel install --user --name="man_ccia" --display-name="man_ccia"

Your new environment should now be available as "man_ccia" after re-launching Jupyter Notebook.
```
...
If you want to use R from within Python, you could add R to an existing environment instead: 
