...
_Last update: `r system("git log -1 --format=\"%ad (%h)\" -- 07-Lab3.Rmd")`_
...
`nc_lat_bnds = nc.variables["lat_bnds"][:].data`<br>
`lat_cell = np.where((nc_lat_bnds[:,0] < target['lat']) & (nc_lat_bnds[:,1] > target['lat']))[0][0]`<br>
`nc_lon_bnds = nc.variables["lon_bnds"][:].data`<br>
...
```{python}
plt.plot(df.Date[df.Experiment == "rcp26"], df.MeanTemp[df.Experiment == "rcp45"], 'y-', alpha=0.7, label = 'RCP 2.6')
