...
Now that we have acquired our GCM output data (Lab 2, Sec. \@ref(lab2)), we need to know how to access it and begin to analyze it. In this lab, you will find a spatial and temporal subset of your GCM data matching your observed climate baseline, and will begin to understand the future climate using change factors. By the end of this lab, you will have data from more than 40 GCMs. In Lab 4 (Sec. \@ref(modelselection) and \@ref(lab4)), we will see how we can narrow-down our selection of the models that will inform our climate change impact assessment. We are going to begin by exploring some of the attributes of three dimensional data contained in the downloaded netCDF files. We are going to use the **netCDF4** module to access the information in our netCDF files. It is strongly recommended that you have a browser tab open with the **netCDF4** library [documentation](http://unidata.github.io/netcdf4-python/) for reference.
...
Let's make sure that we got the right amount of time and data: 12 months times 25 years for 300 data points.
...
Throughout the labs in this book, we have focused our analysis on a single spatial point (or grid cell). **pandas** makes this easy. As you gain skill and as the spatial focus of your research broadens, you may become interested in performing analysis across multiple spatial points, and you will find that the **pandas** code here cannot be easily generalized across dimensions. The powerful **xarray** package borrows concepts from **pandas** and applies them to $n$-dimensional arrays. To learn more about **xarray** refer to the [documentation](https://xarray.pydata.org/en/stable/getting-started-guide/index.html). Another fantastic resouce is the book [_An Introduction to Earth and Environmental Data Science_](https://earth-env-data-science.github.io/intro.html) by Ryan Abernathey, which includes a [chapter](https://earth-env-data-science.github.io/lectures/xarray/xarray_intro.html) introducing **xarray**.
```
...
```{python l3f1, deco=list(), results='hold', fig.cap="Monthly $T_\\mathrm{mean}$ at Toronto (1981\u20122005), as simulated by CanESM2."}
...
The table that we have created above, is a "long" data frame. A long data frame is what is referred to as "[tidy data](https://www.jstatsoft.org/article/view/v059i10)" in the R universe. Tidy data is characterized by a few simple principles: each column is a variable, each row is an observation, each table contains a unique type of observational unit.
...
```{python l3f2, deco=list(), results='hold', fig.cap="CanESM2 Monthly $T_\\mathrm{mean}$ at Toronto (2006‒2040) using three RCP scenarios"}
df.plot()
```

Somewhere in Figure \@ref(fig:l3f2) is the CanESM2 Monthly $T_\mathrm{mean}$ at Toronto (2006‒2040) using our three RCP scenarios, but it looks like we need some more sophisticated code to plot this! We can manually fix the above by filtering for each projection and adding the line manually.

```{python l3f3, deco=list(), results='hold', fig.cap="Monthly $T_\\mathrm{mean}$ at Toronto (2006\u20122040), as simulated by CanESM2 using three RCP scenarios."}
...
What if we want to include the "historical" data? In Figure \@ref{l3f3} we manually plotted each scenario as a separate line on the same plot. The **seaborn** library can also make this process a little easier. Using **seaborn** we can identify what it is that makes each data series unique and how we want to differentiate between series. We will pass `hue="Experiment"` to our **seaborn** plot, which indicates that we want a different hue (i.e. colour) for each unique value in `"Experiment"`.

```{python l3f4, deco=list(), results='hold', fig.cap="Historical and future monthly $T_\\mathrm{mean}$ at Toronto (1981\u20122040), as simulated by CanESM2. Future projections are forced using three RCP scenarios."}
import seaborn as sns
sns.lineplot(data=df, x="Date", y="MeanTemp", hue="Experiment").set(ylabel="Mean Temperature (°C)", title=r'CanESM2 Monthly $T_\mathrm{mean}$ at Toronto (1981‒2040)')
```
...
```{python l3f5, deco=list(), results='hold', fig.cap="Historical and future monthly $T_\\mathrm{mean}$ at Toronto (1981\u20122040), as simulated by CanESM2. Future projections are forced using three RCP scenarios."}
...
```{python, deco=list()}
pd.DataFrame({"Model": np.repeat("CanESM2", 3),
...
- Complete a table similar to the one above for a different model (This can be the model that you used in Lab 2 or you can see a list of model names to choose from in [_Conjuntool_](https://shiny.conr.ca/conjuntool/)). [2 marks]
- The table, below, provides the baseline mean temperature (°C) and 2020s change factor for RCP 2.6 and RCP 4.5 for all five available ensemble members of CanESM2. In some cases, the change factor for the RCP 2.6 scenario is _higher_ than the RCP 4.5 scenario. What are some possible explanations for this? _Hint: Consider the sources of uncertainty in climate model projections and how these relate to two key terms, "pathway" and "ensemble member"._ [2 marks]
...
- Using [_Conjuntool_](https://shiny.conr.ca/conjuntool/), create a _.csv_ file for all of the models that are available for ensemble r1i1p1 for annual mean temperature, _tas_.
  - Click on the "GCM Anomalies" tab. Note that _Anomalies_ in _Conjuntool_ are the same thing as _Change Factors_
...
- Compare the output of the models from _Conjuntool_. Are the model projections (the change factors/anomalies) largely similar, or different? What might explain some of these similarities or differences (consider similarities/differences across scenarios, across time periods and across models). [2 marks]
- Provide a brief summary of the _Conjuntool_ output. Which model simulates the warmest and coldest baseline? Which model projects the largest change in temperatures? Which projects the smallest? What is the multi-model mean baseline and change factor for each period and scenario? Compile your data in a table(s). Think about how best to present this data in tabular form. If you would like to analyze the data using python, Sec. \@ref(lab4) offers some guidance! [4 marks]
```
