...
You are probably most accustomed to working with two-dimensional data sets. In 2-D (rectangular) data, we have a mix of two dimensions of variation. In the case of the data that we collected in Lab `r which(grepl("eccc", list.files(pattern = ".*_lab.Rmd")))`, we can consider our two dimensions as time (T) and variable. Another common pair of dimensions are latitude and longitude. (You will see this when you generate an "Overlay Map" on _Conjuntool_ in Lab `r which(grepl("esgf", list.files(pattern = ".*_lab.Rmd")))` (Sec. \@ref(esgflab)). This is a great example of 2-D data: we include latitude, and longitude, but removed the time dimension by averaging. Refer to Figure \@ref(fig:2d) for a visualization of 2-D data.
...
In Lab `r which(grepl("dims", list.files(pattern = ".*_lab.Rmd")))` (Sec. \@ref(dimslab)), you will learn how to read data from a netCDF file in Python. In that lab, we will focus on a single grid box, but keep in mind that the netCDF files that you will acquire in Lab `r which(grepl("esgf", list.files(pattern = ".*_lab.Rmd")))` contain much more data, and you can re-use the same files for future analyses of other areas, entire regions or even the entire globe!
