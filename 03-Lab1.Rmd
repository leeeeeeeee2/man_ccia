...
```{block, type="rmdtip"}
The **ec3** module returns historical climate data in a data structure called a `DataFrame` (or data frame), which comes from the **pandas** library. A data frame is a tabular data object, and includes features that users of spreadsheet might be used to, like labelled columns. **pandas** provides us with a lot of power and flexibility to organized, modify, and plot our data. It is particularly useful for time series, like our data. For more information on **pandas**, refer to the [documentation](https://pandas.pydata.org/pandas-docs/stable/index.html).
```
...
- Include the time series plots with fitted trend lines for your climate index and for the relevant temperature variable, e.g. `MeanTemp` for freezing degree days. [2 marks] 
- Include a table with the summary statistics for your climate index and temperature variable. [2 marks] 
- Now that we know that Station 5051 was recoded sometime between 2002 and 2003, future analyses might be quicker if we simply merge the 5051 data up to 2002 and the 31688 data from 2003 onward. Does this change in the join date between the two stations change the analysis of your climate index? [2 marks] - Be sure to clearly label your plots and table and include concise figure and table captions. [2 marks]
```
