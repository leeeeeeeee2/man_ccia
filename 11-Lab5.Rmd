...
```{r l5f1, fig.cap="Toronto $T_{mean}$ (1981\u20122100), comparison between observed station data (yellow) and simulation by a multimodel ensemble using downscaled GCM data."}
...
```{r l5f2, fig.cap="Toronto $T_{min}$ (1981\u20122100), comparison between observed station data (yellow) and simulation by a multimodel ensemble using downscaled GCM data."}
...
```{r l5f3, fig.cap="Toronto $T_{max}$ (1981\u20122100), comparison between observed station data (yellow) and simulation by a multimodel ensemble using downscaled GCM data."}
...
```{r l5f4, fig.cap="$T_{mean}$ bias in downscaled multi-model ensemble using downscaled GCM data. The ensemble mean is shown as a black line. Shaded regions indicate one standard deviation (light grey), and the full range of individual ensemble-members (dark grey)."}
knitr::include_graphics("l5f4.png", dpi = NA)
```

## Re-assessing the future

Let's take a look at how the downscaled data projects the future climate. We will perform our analysis using the same tridecades that we used in Lab 4. In the interest of expediency, we have defined a function to bin a given date into one of our four tridecades, and we'll apply the function to our `df.Date` column.
...
```{r l5f5, fig.cap="Simulated and projected 30-year maximum (orange), minimum (green), and mean (blue) temperatures averages at Toronto for tridecades from the 1990s to the 2080s, as projected by a downscaled multimodel ensemble for RCP4.5 (solid) and RCP8.5 (dashed)."}
...
```{r l5f6, fig.cap="Simulated and projected 30-year mean temperatures averages at Toronto for tridecades from the 1990s to the 2080s, as projected by six downscaled GCMs for RCP4.5 (solid) and RCP8.5 (dashed)."}
...
```{r l5f7, fig.cap="Simulated and projected 30-year mean temperatures averages at Toronto for tridecades from the 1990s to the 2080s, as projected by six downscaled GCMs for RCP4.5 (top) and RCP8.5 (bottom)."}
...
```{r l5f8, fig.cap="Simulated and projected 30-year total cooling degree days (CDD) at Toronto for tridecades from the 1990s to the 2080s using downscaled GCMs for RCP4.5 (light) and RCP8.5 (dark)."}
...
```{r l5f9, fig.cap="Simulated and projected 30-year total cooling degree days (CDD) at Toronto for tridecades from the 1990s to the 2080s using downscaled GCMs for RCP4.5 (light) and RCP8.5. The true historical observed CDDs are shown in dark blue."}
