# Statistically downscaled climate data from PCIC {#pcic}
...
In this exercise, we will use data that has been downscaled using a method called "bias correction / constructed analogues with quantile mapping reordering" (BCCAQv2; see @cannon2015bias). One of the advantages of this method is that we can bias-correct and downscale daily GCM data.

The data was provided by @pcic2019statistically, and is downscaled based on a high-quality gridded observed data set (~10 km grid size) developed by NRCan (ANUSPLIN300). The Pacific Climate Impacts Consortium ([PCIC](https://www.pacificclimate.org/)) provides an excellent overview of their downscaling methodology, with citations made to the relevant articles, [here](https://
...
```{python create_l5f0, include=FALSE}
## Downscaled Data
...
```{r l5f0, echo=FALSE, results='hold', fig.cap="Baseline (1981\u20122100) $T_\\mathrm{mean}$ at Toronto, observed (green), simulated by CanESM2 (yellow), and downscaled by PCIC (blue)."}
knitr::include_graphics("l5f0.png", dpi = NA)
```

## Obtaining statistically downscaled PCIC data

There are several places that you can download PCIC's statistically downscaled data. We present four options here, two that use your web browser, and two that you can access directly from Python. 

### Using a web browser to obtain PCIC data

#### Using the PCIC data portal website to obtain PCIC data

PCIC provides an intuitive point-and-click map for accessing the downscaled GCM data. In our exercise in Lab 5, we will use six models (CanESM2, CCSM4, CSIRO-Mk3-6-0, GFDLESM2G, inmcm4 and MIROC5). To download this information manually, follow the steps below. 
...
#### Using ClimateData.ca to obtain PCIC data

[ClimateData.ca](https://climatedata.ca) is a collaboration between the Government of Canada (via ECCC), and several research partners (the Computer Research Institute of Montréal ([CRIM](https://www.crim.ca/en/)), [Ouranos](https://www.ouranos.ca/en/ouranos/), [PCIC](https://www.pacificclimate.org/), the [Prairie Climate Centre](https://prairieclimatecentre.ca/), and [HabitatSeven](https://habitatseven.com/)) to provide a platform for access to climate data other information and tools to support decision-making around climate change. Similar to the PCIC data portal, ClimateData.ca also provides an intuitive point-and-click map for accessing the PCIC BCCAQv2 data. To download this information manually, follow the steps below. 
...
7. Note that the ClimateData.ca download page does not allow you to select models or scenarios. Your download will include all available models and scenarios for the PCIC BCCAQv2 data and it will include separate files for each type of daily calendar (e.g. some GCMs include leap years and some do not).
...
### Using Python to obtain PCIC data

#### Using PAVICS to obtain PCIC data

Power Analytics and Visualization for Climate Science ([PAVICS](https://pavics.ouranos.ca/index.html)) is a new research platform dedicated to climate analysis and visualization. It bundles data search, analytics and visualization services. PAVICS is developed by [Ouranos](https://www.ouranos.ca/en/ouranos/), [CRIM](https://www.crim.ca/en/) and the [birdhouse community](http://bird-house.github.io/) and been funded by the [CANARIE](https://www.canarie.ca/software/platforms/pavics/) research software program. We can use Python to access the PAVICS server to download the PCIC BCCAQc2 data.
...
#### Obtaining PCIC data programmatically
...
## Ready to analyze downscaled PCIC data

Now that you have the data, we can proceed to analyze it. In Lab 5 (Sec. \@ref(lab5)), we will compare the downscaled GCM data to our observed data and re-assess our climate index based on these new data.
