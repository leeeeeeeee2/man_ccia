...
You will notice that we have multiple URLs for the same file. We don't need to download each file multiple times, so you can choose to drop the duplicates using `files.drop_duplicates('filename')`.
...
- Given a file named _tas_Amon_MIROC5_rcp85_r1i1p1_200601-210012.nc_, what can you ascertain about the data the file contains? [2 marks]
...
- To save you time and hard drive space, we will be using _Conjuntool_ to pre-process and subset our data. Visit [_Conjuntool_](https://shiny.conr.ca/conjuntool/) and familiarize yourself with the interface. For the model corresponding to the file named in question 2:
  - Export a time series plot of the entire range of the data (go to the GCM Plots option in the left side-bar). Select the _Annual_ Timeframe option. Pick any location within Canada.
  - Create overlay maps for two periods: 2011 to 2040 and 2071 to 2100 (go to the Overlay Maps option in the left side-bar).  [2 marks]
