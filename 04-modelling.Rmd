...
In Lab `r which(grepl("eccc", list.files(pattern = ".*_lab.Rmd")))`, we learned how to acquire data for the past, and perform a basic baseline analysis. In the other labs we will be looking the other direction, towards the future. Before we begin, this chapter serves as a primer on (or, hopefully, review of) basic climate modelling.
...
If you would like to see some examples of the four RCPs and climate model data, poke around the "plots" tab of [_Conjuntool_](https://shiny.conr.ca/conjuntool/), which we will be using in upcoming labs. In Lab `r which(grepl("esgf", list.files(pattern = ".*_lab.Rmd")))` (Sec. \@ref(esgflab)), you will learn how to obtain GCM data from the Earth System Grid Federation (ESGF). In Lab `r which(grepl("dims", list.files(pattern = ".*_lab.Rmd")))` (Sec. \@ref(dimslab)), we will learn to read multi-dimsensional data from files like these into Python. 
