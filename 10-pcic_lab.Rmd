# Lab `r which(grepl("pcic", list.files(pattern = ".*_lab.Rmd")))`: Using Downscaled GCM Data {#pciclab}

_Last update: `r system("git log -1 --format=\"%ad (%h)\" -- 10-pcic_lab.Rmd", intern = TRUE)`_
...
In Lab `r which(grepl("select", list.files(pattern = ".*_lab.Rmd")))` (Sec. \@ref(selectlab)) we applied GCM-derived change factors to our observed data as a rudimentary form of downscaling, and removing the bias in the model projections. In that sense, we kept the relative change in the model projections, but discarded the monthly and seasonal signals. In this lab, we will use downscaled data provided by @pcic2014statistically. We downloaded these files, in Section \@ref(pcic).
...
Let's see how the downscaled data compare to our observed data. Let's read in our observed data that we generated in Lab `r which(grepl("eccc", list.files(pattern = ".*_lab.Rmd")))` (Sec. \@ref(eccclab)), and generate the same stats.
...
Let's take a look at how the downscaled data projects the future climate. We will perform our analysis using the same tridecades that we used in Lab `r which(grepl("select", list.files(pattern = ".*_lab.Rmd")))`. First, we'll filter our data to grab the data from 1981 to 2100, which covers our baseline and our three tridecades. Next, we'll add an empty `"Period"` column, and classify our tridecades by masking the relevant periods and assigning a tridecade name.
...
- How do the results of the downscaled GCM data compare to the change-factor method that you used in Lab `r which(grepl("select", list.files(pattern = ".*_lab.Rmd")))`? [5 marks]
